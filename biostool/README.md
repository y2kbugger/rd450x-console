...
- `modedit` — `replace_module_body` (подмена тела модуля in-place),
  `patch_string_inplace` (UTF-16 строка ≤ длины), `repoint_ref` (FormId в IFR-Ref).
...
```sh
$PY -m biostool.validate    img/R450X219_bmc_nvme.rom   # самотесты (round-trip и т.д.)
$PY -m biostool.build_rd450x                            # -> build/219_claude.rom
# + ReBar:
$PY -m biostool.cli insert-ffs build/219_claude.rom build/ReBarDxe.ffs build/219_rc_rebar.rom --volume 2
```
